-- @description Duplicate selected MIDI events 2 bars later (Skip a Bar)
-- @version 1.0
-- @author vibe coded with Gemini 

function DuplicateTwoBarsLater()
    ...
    if not has_selection then 
        reaper.PreventUIRefresh(-1)
        reaper.Undo_EndBlock('Duplicate Two Bars Later', -1)
        return 
    end

    -- 3. CALCULATE OFFSET (2 BARS LATER)
    ...
    if is_pickup then
        -- Pickup Logic: We are technically leading into Bar N+1. 
        -- We want to lead into Bar N+3.
        -- Offset = (Start of Bar N+3) - (Start of Bar N+1)
        local meas_plus_1 = reaper.TimeMap_GetMeasureInfo(0, measure_idx + 1)
        local meas_plus_3 = reaper.TimeMap_GetMeasureInfo(0, measure_idx + 3)
        offset_seconds = meas_plus_3 - meas_plus_1
    else
        -- Standard Logic: We are in Bar N.
        -- We want to land in Bar N+2.
        -- Offset = (Start of Bar N+2) - (Start of Bar N)
        local meas_plus_2 = reaper.TimeMap_GetMeasureInfo(0, measure_idx + 2)
        offset_seconds = meas_plus_2 - curr_meas_start
    end
